Meteorite, Copyright 2017
Meteorite, is licensed under GNU General Public License V3 or later. You can find a copy of it in the license.txt file.


Installation
1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.


Supported plugins
Meteorite includes support for Page Builder by SiteOrigin, Terra Themes Tools and Meteorite Extensions.


Changelog
= 1.7 | 04/29/2017 =
* Fixed a translation bug for comments
* Improved style.css file size
* Removed wp_list_comments callback in comments.php
* Updated meteorite.pot file

= 1.6 | 04/20/2017 =
* Added more functionality to overwrite theme functions in child themes
* Added some styles in combination with the plugin update from Terra Themes Tools 1.2
* Improved data escaping
* Removed styles for Terra Slider pause icon
* Updated meteorite.pot file

= 1.5 | 04/10/2017 =
* Added functionality to overwrite functions from inc/functions/functions-header.php in child themes
* Fixed a conflict between .smooth-scroll and outgoing links

= 1.4 | 03/24/2017 = 
* Added .smooth-scroll to arrow buttons in header image
* Changed featured pricing table style
* Changed font system
* Fixed missing 1.2.1 updates that were missing in 1.3
* Fixed a bug where the mobile nav gets positioned wrong under specific circumstances
* Fixed a bug with Menu style inline
* Improved CSS styles
* Removed timeline support
* Updated meteorite.pot file

= 1.3 | 03/16/2017 =
* Added Meteorite Extensions Plugin as recommended
* Improved some styles for the image frame widget

= 1.2.1 | 03/12/2017 =
* Fixed bug with navicon on screens between 1024px and 992px
* Removed favicon upload in customizer - use site icon instead

= 1.2 | 03/11/2017 =
* Fixed animation JS for text with icon widget
* Improved diverse PHP issues
* Updated translation file
* Updated theme license to GPLv3

= 1.1 =
* Added light logo functionality
* Added fullheight row options
* Added action hooks
* Added text with icon widget type and animations
* Changed screenshot.png a bit
* Improved escaping in widgets
* Improved CSS styles
* Improved titlebar
* Improved header container for long site titles
* Updated Font Awesome

= 1.0.6 =
* Several changes

= 1.0 =
* Initial release


Credits

a) Underscores
Meteorite is based on Underscores 
Resource URI: http://underscores.me/
Copyright: 2012-2016 Automattic, Inc., 
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

b) Bootstrap 
Resource URI: http://getbootstrap.com/
Copyright: 2011-2015 Twitter, Inc
License: MIT
License URI: http://opensource.org/licenses/MIT

c) FitVids 
Resource URI: http://fitvidsjs.com/
Copyright: Chris Coyier, Paravel
License: WTFPL
License URI: http://www.wtfpl.net/txt/copying/

d) html5shiv 
Resource URI: https://code.google.com/p/html5shiv/
Copyright: Alexander Farkas, Jonathan Neal, Paul Irish
License: MIT
License URI: http://opensource.org/licenses/MIT

e) Isotope 
Resource URI: http://isotope.metafizzy.co/
Copyright: Metafizzy
License: GPLv3
License URI: https://www.gnu.org/licenses/gpl-3.0.html

f) Owlcarousel 
Resource URI: http://owlgraphic.com/owlcarousel/
Copyright: Bartosz Wojciechowski
License: MIT
License URI: http://opensource.org/licenses/MIT

g) jQuery Waypoints 
Resource URI: http://imakewebthings.com/jquery-waypoints/
Copyright: Caleb Troughton
License: MIT
License URI: http://opensource.org/licenses/MIT

h) jQuery CountTo
Resource URI: https://github.com/mhuggins/jquery-countTo
Copyright: Matt Huggins
License: MIT
License URI: http://opensource.org/licenses/MIT

i) Icon set used is FontAwesome 
Resource URI: http://fontawesome.io
Copyright: Dave Gandy
License: SIL Open Font License | MIT
License URI: https://scripts.sil.org/OFL | http://opensource.org/licenses/MIT

j) easy-pie-chart 
Resource URI: https://rendro.github.io/easy-pie-chart/
Copyright: Robert Fleischmann
License: MIT
License URI: http://opensource.org/licenses/MIT

k) MediaElement.js 
Resource URI: http://mediaelementjs.com/
Copyright: John Dyer
License: MIT
License URI: http://opensource.org/licenses/MIT

l) Image used in screenshot 
Resource URI: https://unsplash.com/photos/Sdc35GnHgvI
Copyright: Vincent Guth
License: CC0
License URI: https://creativecommons.org/publicdomain/zero/1.0/