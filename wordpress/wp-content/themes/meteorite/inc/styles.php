...
function meteorite_custom_styles($custom) {
	...
	if ( $header_image_full_height_front == 1 && is_front_page() ) {
		...
		$custom .= "@media only screen and (min-width: 992px) { .home .has-header #masthead.above:not(.above-solid) .topbar, .home .has-header #masthead.above:not(.above-solid) .nav-container { background-color: transparent; } }"."\n";
		$custom .= "@media only screen and (max-width: 991px ) { .home #masthead { position: relative !important; } .home .parallax-text { position: absolute !important; top: 50% !important; -webkit-transform: translate(-50%, -50%); -ms-transofrorm: translate(-50%, -50%); transform: translate(-50%, -50%); } }"."\n";
	} elseif ( $header_height_home != 800 && is_front_page() ) {
		$custom .= ".home .header-image { height:" . intval($header_height_home) . "px; }"."\n";
	}

	//Header image sub pages
	if ( $header_image_full_height_page == 1 && is_page() ) {
		$custom .= "@media only screen and (min-width: 992px) { .page:not(.home) .header-area, .page:not(.home) .header-image { min-height: 100vh; } }"."\n";
		$custom .= "@media only screen and (min-width: 992px) { .page:not(.home) .has-header #masthead.above:not(.above-solid) .topbar, .page:not(.home) .has-header #masthead.above:not(.above-solid) .nav-container { background-color: transparent; } }"."\n";
		$custom .= "@media only screen and (max-width: 991px ) { .page:not(.home) #masthead { position: relative !important; } .page:not(.home) .parallax-text { position: absolute !important; top: 50% !important; -webkit-transform: translate(-50%, -50%); -ms-transofrorm: translate(-50%, -50%); transform: translate(-50%, -50%); } }"."\n";
	} elseif ( $header_height_page != 800 && is_page() ) {
		$custom .= ".page:not(.home) .header-image { height:" . intval($header_height_page) . "px; }"."\n";
	}	
	...
	} elseif ( $header_image_full_height_front != 1 && is_front_page() || $header_image_full_height_page != 1 && is_page() && !is_front_page() ) {
		$custom .= "@media only screen and (min-width: 992px) { .has-header #masthead.above:not(.above-solid) .topbar, .has-header #masthead.above:not(.above-solid) .nav-container { background-color: transparent; } }"."\n";
	}
	...
	if ( $menu_position == 'above' ) {
		$custom .= "#masthead { position: absolute; top: 0; width: 100%; z-index: 750; }"."\n";
	}
