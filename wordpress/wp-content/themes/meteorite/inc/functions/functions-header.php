...
function meteorite_has_header() {
	...
	if ( 
		get_theme_mod('header_image_active', 1) == 1 && (
		shortcode_exists('terra-themes-header-slider') && !empty( $terra_themes_header_slider_shortcode ) 
		|| !is_single() && !is_archive() && !is_search() && !is_404() && !is_home()
	...
	if ( get_theme_mod('header_image_active', 1) == 1 && ( shortcode_exists('terra-themes-header-slider') && !empty($terra_themes_header_slider_shortcode) ) ) {
		echo ' has-slider';
	}
}
endif;
...
function meteorite_check_header() {
	...
	if ( is_home() ) {
		$terra_themes_header_slider_shortcode = get_post_meta(get_queried_object_id(), '_terra_themes_header_slider', true);
...
function meteorite_header_titlebar() {
	if ( get_theme_mod('header_titlebar', 'off') == 'on' && ! is_front_page() && meteorite_check_header() != 'has-header' ) : ?>
		<div class="titlebar">
			<div class="container">
				<div class="titlebar-content clearfix">
					...
					elseif ( is_search() ) :
						echo '<h1>' . printf( esc_html__( 'Search Results for: %s', 'meteorite' ), '<span>' . get_search_query() . '</span>' ) . '</h1>';

					elseif ( is_404() ) :
						echo '<h1>' . esc_html( 'Oops! That page can&rsquo;t be found.', 'meteorite' ) . '</h1>';
...
function meteorite_header_hero_area() {
	...
	if ( get_theme_mod('header_image_active', 1) == 1 && meteorite_check_header() == 'has-header' ) :
		...
		} elseif ( is_front_page() || is_page() ) {																				// Image
				...
				<div class="header-container <?php echo $responsive_header_image; ?>">
					<div class="parallax-header header-image" style="background-image: url('<?php echo esc_url($src); ?>');">
						...
						<div class="parallax-text container">
							<?php if ( $header_image_title ) { echo '<' . $title_tag . ' class="header-image-heading">' . esc_html($header_image_title) . '</' . $title_tag . '>'; } ?>
							<?php if ( $header_image_text ) { echo '<' . $text_tag . ' class="header-image-text">' . esc_html($header_image_text) . '</' . $text_tag . '>'; } ?>
							...
							<div class="header-cta-buttons">
								...
								if ( !empty($cta_button_text_two) ) { ?>
									<a href="<?php echo esc_url($cta_button_link_two); ?>" class="header-cta-one meteorite-button border"><?php echo esc_html($cta_button_text_two); ?></a>
								<?php } ?>
							</div>
							<?php } ?>
						</div>
